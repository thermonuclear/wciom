...
/**
 * QRCODE module for Kohana
 * Based in PHPQRcode project (http://phpqrcode.sourceforge.net)
 *
 * @package    QRBarcode
 * @author     Juan Lago D. <juanparati[at]gmail[dot]com>
 * @author     Dominik Dzienia <deltalab[at]poczta[dot]fm>
...
abstract class Kohana_QRCode {
  ...
  public function encodeMask(QRinput $input, $mask)
  {
      if($input->getVersion() < 0 || $input->getVersion() > QRConst::QRSPEC_VERSION_MAX) {
        throw new Kohana_Exception('Wrong version of QRCode');
      }
      if($input->getErrorCorrectionLevel() > QRConst::QR_ECLEVEL_H) {
        throw new Kohana_Exception('Wrong level in QRCode');
      }
  ...
  {
    if(string == NULL) {
      throw new Kohana_Exception('Empty string');
      return NULL;
    }
  ...
  public function encodeString($string, $version, $level, $hint, $casesensitive)
  {
    ...
    if($hint != QRConst::QR_MODE_8 && $hint != QRConst::QR_MODE_KANJI) {
      throw new Kohana_Exception('bad hint');
      return NULL;
    }
  ...
  /**
	 * Generate or display a QRcode
	 *
	 * @params   string   String to encode
	 * @params   mixed    File output or false for direct display
	 * @params   integer  QRCode error correction and quality (Default 3 = Max quality)
  ...
  /**
	 ...
	 *
	 * @chainable
	 * @params   string   Configuration key
	 * @params   mixed    Value
	 * @return   object
	 */
  ...
  /**
	 * Generate or display as binary matrix
	 *
	 * @params   string   String to encode
	 * @params   mixed    File output or false for direct display
	 * @params   integer  QRCode error correction and quality (Default 1 = Low quality)
	 * @params   integer  Size (Default 3)
	 * @params   integer  Margin (Default 4)
	 * @return   array
	 */
...
class QRtools {
  ...
  public static function tcpdfBarcodeArray($code, $mode = 'QR,L', $tcPdfVersion = '4.5.037')
  {
      ...
      if (is_iterable($mode) && count($mode) > 1)  {
          $eccLevel = $mode[1];
      }
  ...
  public static function buildCache()
  {
    ...
    for ($a=1; $a <= QRConst::QRSPEC_VERSION_MAX; $a++)
    {
      ...
      if (QRConst::QR_IMAGE) {
        $fileName = APPPATH.'cache'.DIRECTORY_SEPARATOR.'frame_'.$a.'.png';
...
class QRspec {
  ...
  {
      if (is_iterable($spec) && count($spec))  {
  ...
  public static function newFrame($version)
  {
    ...
    if(!isset(self::$frames[$version])) {
      ...
      if (Kohana::$config->load('qrcode')->cacheable) {
        ...
        if (empty(self::$frames[$version])) {
          self::$frames[$version] = self::createFrame($version);
          Kohana::cache('frame_'.$version.'.dat', self::serial(self::$frames[$version]));
        }
        else
          self::$frames[$version] = self::unserial(self::$frames[$version]);
...
class QRimage {
  ...
  public static function png($frame, $filename = FALSE, $pixelPerPoint = 4, $outerFrame = 4,$saveandprint=FALSE)
  {
      ...
      if ($filename === FALSE)
      {
          Request::current()->headers['Content-Type'] = 'image/png';
          ImagePng($image);
      }
      else
      {
  ...
  private static function image($frame, $pixelPerPoint = 4, $outerFrame = 4)
  {
      ...
      foreach ($frame as $str) {
          $min = $min > strlen($str) ? strlen($str) : $min;
      }
      $h = count($frame);
      $w = $min;
...
class QRinputItem {
  ...
  public function __construct($mode, $size, $data, $bstream = null)
  {
      ...
      if(!QRinput::check($mode, $size, $setData)) {
        throw new Kohana_Exception('Error m:'.$mode.',s:'.$size.',d:'.join(',',$setData));
        return null;
      }
...
class QRinput {
  ...
  public function __construct($version = 0, $level = QRConst::QR_ECLEVEL_L)
  {
    if ($version < 0 || $version > QRConst::QRSPEC_VERSION_MAX || $level > QRConst::QR_ECLEVEL_H) {
      throw new Kohana_Exception('Invalid version no in QRCode');
      return NULL;
    }
  ...
  public function setVersion($version)
  {
    if($version < 0 || $version > QRConst::QRSPEC_VERSION_MAX) {
      throw new Kohana_Exception('Invalid version in QRCode');
      return -1;
    }
  ...
  public function setErrorCorrectionLevel($level)
  {
    if($level > QRConst::QR_ECLEVEL_H) {
      throw new Kohana_Exception('Invalid ECLEVEL');
      return -1;
    }
  ...
  public function insertStructuredAppendHeader($size, $index, $parity)
  {
      if( $size > QRConst::MAX_STRUCTURED_SYMBOLS ) {
        throw new Kohana_Exception('insertStructuredAppendHeader wrong size');
      }

      if( $index <= 0 || $index > QRConst::MAX_STRUCTURED_SYMBOLS ) {
        throw new Kohana_Exception('insertStructuredAppendHeader wrong index');
      }
  ...
  public function convertData()
  {
      ...
      for(;;) {
          ...
          if($ver < 0) {
            throw new Kohana_Exception('Wrong version in QRCode');
            return -1;
...
class QRsplit {
  ...
  public static function splitStringToQRinput($string, QRinput $input, $modeHint, $casesensitive = true)
  {
      if(is_null($string) || $string == '\0' || $string == '') {
        throw new Kohana_Exception('Empty string in QRCode');
      }
  ...
  public function makeMaskNo($maskNo, $width, $s, &$d, $maskGenOnly = false)
  {
      ...
      if (Kohana::$config->load('qrcode')->cacheable) {
          ...
          if (empty($bitMask))
          {
            ...
            Kohana::cache($fileName, self::serial($bitMask));
          }
          else
            $bitMask = self::unserial($bitMask);
...
class QRrawcode {
  ...
  public function __construct(QRinput $input)
  {
      ...
      if(is_null($this->datacode)) {
        throw new Kohana_Exception('NULL input string in QRCode');
      }
      ...
      $ret = $this->init($spec);
      if($ret < 0) {
        throw new Kohana_Exception('Block alloc error');
        return null;
      }
