...
class Kohana_Pagination {
    ...
    protected $max_left_pages;
    protected $max_right_pages;
	...
	public function setup(array $config = [])
	{
			...
			OR isset($config['items_per_page']))
		{
            ...
            $cubes_count = 0;
            if ($this->current_page != 1) $cubes_count += 2;
            if ($this->current_page != $this->total_pages) $cubes_count += 2;
            $div2 = floor ((Kohana::$config->load('site')->pagination_cubes_count - $cubes_count) / 2);
            if ($this->current_page - 1 < $this->total_pages - $this->current_page)
            {
                $this->max_left_pages = $div2;
                $cubes_count += min($div2, $this->current_page);
                $this->max_right_pages = Kohana::$config->load('site')->pagination_cubes_count - $cubes_count;
            } else
            {
                $this->max_right_pages = $div2;
                $cubes_count += min($div2, $this->total_pages - $this->current_page);
                $this->max_left_pages = Kohana::$config->load('site')->pagination_cubes_count - $cubes_count;
            }
		}
